...
func _ready() -> void:
	...
	run.call_deferred()
func run() -> void:
	level = load("res://src/levels/level_02.tscn").instantiate()
	add_child(level)
	...
	DirAccess.make_dir_recursive_absolute("/tmp/colossus-fight-demo")
	running = true
	var ending := 0
	for frame in range(540):
		...
		get_viewport().get_texture().get_image().save_png("/tmp/colossus-fight-demo/frame_%04d.png" % frame)
