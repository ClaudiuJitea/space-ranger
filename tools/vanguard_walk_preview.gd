...
func capture() -> void:
	...
	camera.position = Vector3(level.arena_entry + 15, 5.5, 16)
	camera.fov = 36
	camera.look_at(Vector3(level.arena_entry + 15, 2.2, 0))
	DirAccess.make_dir_recursive_absolute("/tmp/vanguard-walk")
	...
	for frame in range(55):
		...
		get_viewport().get_texture().get_image().save_png("/tmp/vanguard-walk/frame_%03d.png" % frame)
